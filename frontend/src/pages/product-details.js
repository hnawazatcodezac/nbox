...
function ProductDetails() {
  ...
  const fetchProduct = async () => {
    try {
      const response = await fetch(
        `${serverBaseUrl}/merchant/product/details/${productId}`,
        {
          method: "GET",
          headers: {
      ...
      if (response.ok) {
        setProduct(responseData.response?.data || []);
        setType(responseData.response?.data?.optionType || []);
        setVariants(responseData.response?.data?.variants || []);
        setOldImages(responseData.response?.data?.images || []);
        setDeleteImages([]);
      } else {
  ...
  const handleProductSubmit = async (values, { setSubmitting, resetForm }) => {
    ...
    try {
      ...
      const variantsWithoutId = variants.map(({ _id, ...rest }) => rest);
      const sanitizeUrlsArray = (urlsArray) => {
        return urlsArray.map((url) => encodeURI(url.trim())).join(",");
      };
      ...
      formData.append("oldImages", sanitizeUrlsArray(oldImages));
      ...
      if (values.enableOption) {
        ...
        formData.append("variants", JSON.stringify(variantsWithoutId));
      }
  ...
  const removeOldImage = (index) => {
    setDeleteImages((prev) => [...prev, oldImages[index]]);
      ...
      <div className="form-container">
        <Formik
          initialValues={{
            ...
            chargeTax: product?.chargeTax ? true : false,
            chargeTaxValue: product?.chargeTax,
            status: product?.status,
            availability: product?.availability,
            orderQuantity: product?.minQuantity ? true : false,
            minQuantity: product?.minQuantity,
            maxQuantity: product?.maxQuantity,
            enableInventory: product?.inventory ? true : false,
            inventory: product?.inventory,
            labels: product?.labels,
            tags: product?.tags,
            enableOption: product?.enOptionName ? true : false,
            enOptionName: product?.enOptionName,
            frOptionName: product?.frOptionName,
            optionType: product?.optionType,
            optionRequired: product?.optionRequired,
          }}
